...
console.log(text.replace(/Manuel Marques de Sousa/, 'Tiago Mariani Palte'));
...
function getMonthNumber(month) {
  ...
  var index = months.indexOf(month.toLowerCase()) + 1;
  if(index === 0)
    return '';
  else if (index < 10)
    return '0' + index;
...
var regexDate = /(\d\d) de ([\w]{1,}) de (\d{1,4})/;
...
console.log(text.replace(/(\d\d) de ([\w]{1,}) de (\d{1,4})/g, replaceDate));
